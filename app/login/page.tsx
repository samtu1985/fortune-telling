...
function isInAppBrowser(): boolean {
  ...
  // Detect common in-app browsers (LINE, FB, IG, WeChat, etc.)
...
export default function LoginPage() {
  ...
  const handleOpenExternal = () => {
    ...
    // For LINE: use line://nv/externalBrowser
    // For others: use window.open or intent
    const ua = navigator.userAgent || "";
    if (/Line/i.test(ua)) {
      window.location.href = `https://line.me/R/browse?url=${encodeURIComponent(currentUrl)}`;
    } else {
      // Generic: open in new window (works for some in-app browsers)
          ...
          <>
            <div className="mx-auto max-w-xs space-y-4 text-sm text-mist/80">
              ...
              <button
                ...
                className="
                  mx-auto px-8 py-3.5 min-h-[44px]
